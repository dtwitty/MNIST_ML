...
void KNNModel::SetRegression(bool is_regression) { regression = is_regression; }

void KNNModel::SetMaxK(int maximum_k) { max_k = maximum_k; }

void KNNModel::Train(const cv::Mat& training_vectors,
                     const cv::Mat& training_labels) {
  knn_->train(training_vectors, training_labels, cv::Mat(), regression, max_k,
              false);
}
...
void KNNModel::PredictK(const cv::Mat& test_vectors, cv::Mat* predicted_labels,
                        int k) {
